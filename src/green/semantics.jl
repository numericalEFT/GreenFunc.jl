"""
    Gf(ma::MeshArray; target_ndim=0, statistics, component=:generic,
       target_labels=nothing, metadata=NamedTuple())
...
"""
struct Gf{A<:MeshArray,L,M<:NamedTuple}
    array::A
    target_ndim::Int
    statistics::Bool
    component::Symbol
...
function Gf(ma::MeshArray;
    ...
    _validate_mesh_statistics(ma.mesh[(target_ndim + 1):end], statistics)
    return Gf{typeof(ma),typeof(labels),typeof(metadata)}(
        ma, target_ndim, statistics, component, labels, metadata,
    )
end

"""
    Gf(meshes...; target_shape=(), data=nothing, dtype=ComplexF64, statistics,
       component=:generic, target_labels=nothing, metadata=NamedTuple())
...
function Gf(meshes...;
    ...
    return Gf(ma;
        ...
        component=component, target_labels=target_labels, metadata=metadata,
    )
end
...
Base.eltype(::Type{Gf{A,L,M}}) where {A,L,M} = eltype(A)
...
Base.propertynames(::Gf, private::Bool=false) = private ?
    (:array, :target_ndim, :statistics, :component, :target_labels, :metadata,
        :data, :mesh, :fullmesh, :dims, :target_shape) :
    (:data, :mesh, :target_shape, :statistics, :component, :target_labels, :metadata)
...
function Base.similar(gf::Gf, ::Type{T}=eltype(gf.array)) where {T}
    return Gf(similar(gf.array, T);
        ...
        component=gf.component, target_labels=gf.target_labels, metadata=gf.metadata,
    )
end
...
Validated matrix-valued spectral density on a mesh containing [`ReFreq`](@ref).
`constraint=:hermitian` checks every complete target matrix;
...
function SpectralDensity(gf::Gf;
    ...
    gf.target_ndim == 2 ||
        throw(ArgumentError("a SpectralDensity requires a matrix target (target_ndim=2)"))
    gf.target_shape[1] == gf.target_shape[2] ||
...
function Base.getproperty(spectral::SpectralDensity, name::Symbol)
    name in (:data, :mesh, :target_shape, :statistics, :component,
        :target_labels, :metadata) && return getproperty(getfield(spectral, :gf), name)
