@testset "GreenFunc" begin
    ...
    @testset "MeshProduct with BrillouinZoneMeshes" begin
        N1 = 5
        mesh1 = MeshGrids.SimpleGrid.Uniform{Float64}([0.0, 1.0], N1)
        N21, N22 = 3, 5
        N2 = N21 * N22
        lattice = Matrix([1/N21/2 0; 0 1.0/N22/2]') .* 2π
        # so that bzmesh[i,j] = (2i-1,2j-1)
        br = MeshGrids.BZMeshes.Cell(lattice=lattice)
        mesh2 = MeshGrids.BZMeshes.UniformBZMesh(cell=br, size=(N21, N22), origin=0)
        meshprod = MeshProduct(mesh1, mesh2)
