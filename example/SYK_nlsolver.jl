...
const mesh_dlr = DLRFreq(β, isFermi; Euv=5.0, rtol=rtol, sym=:ph, rebuild=false)   # Initialize DLR grid

function selfenergy(Gt)
    ######### calculate sigma ###############
    minus_tau = reverse(mesh_dlr.β .- Gt.mesh[1]) # Reversed imaginary time mesh point
    Gt_inv = dlr_to_imtime(to_dlr(Gt, mesh_dlr), minus_tau) # G at beta - tau
    Gmt = reverseview(Gt_inv)
    Σt = J .^ 2 .* Gt .^ 2 .* Gmt  # SYK self-energy in imaginary time
    return dlr_to_imfreq(to_dlr(Σt, mesh_dlr))
end
...
function dyson(Gt)
    ...
    #############  Gω --> Gτ  ###############
    return dlr_to_imtime(to_dlr(Gω, mesh_dlr))
end
...
const mesh = ImTime(β, isFermi; Euv=5.0, grid=mesh_dlr.dlr.τ)
